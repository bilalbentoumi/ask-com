...
@section('title', __('lang.edit_question'))
                        ...
                        <div class="panel-body">
                            <form method="POST" action="{{ route('user.questions.update', $question->id) }}">
                                ...
                                <div class="field {{ $errors->has('category') ? 'has-error' : '' }}">
                                    ...
                                    <select name="category">
                                        @forelse($categories as $category)
                                            <option @if($category->id == $question->category->id) selected @endif value="{{ $category->id }}">{{ $category->name }}</option>
                                        @empty
                                            {{ __('lang.no_data') }}
                                        @endforelse
                                    </select>
                                ...
                                <div class="field {{ $errors->has('tags') ? 'has-error' : '' }}">
                                    ...
                                    <input type="text" class="tags form-control{{ $errors->has('tag-it') ? ' is-invalid' : '' }}" name="tags" value="{{ old('tags', $question->tags_string()) }}">
                                    @if ($errors->has('tags'))
                                ...
                                <div class="field">
                                    ...
                                    <a href="{{ $question->url }}" type="submit" class="btn btn-secondary">
                                        {{ __('lang.cancel') }}
                                    </a>
                                </div>
                            </form>
